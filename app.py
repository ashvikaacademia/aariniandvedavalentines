...
def story_page():
    ...
    with col2:
        st.image("aariniandveda.jpeg",width=500)
...
def quiz():
    ...
    a2=st.text_input(q2)
    q3="🎲 How do you describe yourself in one word?"
    a3=st.text_input(q3)
    q4="🫶🏻 Describe me in one word."
    a4=st.text_input(q4)
    q5="🫀 How do you describe us in one word?"
    a5=st.text_input(q5)

    
    if st.button("I'm done 🪅"):
        if a2 and a3 and a4 and a5:
            st.markdown(f"""
            <style>
            .a{{
            text-align:center;
            ...
            <div class="a"><h4>Our date idea: {a1}. Your favourite memory of us is {a2} (Awwwww). Veda, you describe yourself by {a3} and me by {a4} 🌟 According to you, we can be described by {a5} yayyyyy :P.</h4></div>
            """, unsafe_allow_html=True)
...
def popping_question():
    col1, col2, col3 = st.columns([1, 6, 1]) 
    with col2:
        st.video("Poppingquestion.mp4", format="video/mp4", start_time=0, subtitles=None, end_time=None, loop=False, autoplay=True, muted=False, width=650)
